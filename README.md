...
Watchdog 通过实时分析日志文件来监控 MaaFramework Pipeline 的执行流程。它利用可配置的状态机来跟踪任务流转、检测超时，并在操作未在预期时间内完成时，通过 Telegram 或企业微信发送警报。

## 🌟 功能特性

*   **非侵入式监控**：通过 `maa.log` 文件监控代理，无需注入进程内存。
*   **复杂状态机**：支持多步流转规则（例如：`开始 -> 步骤 1 -> 步骤 2 -> 结束`）。
*   **超时检测**：如果特定节点或流转步骤超过了定义的阈值，自动发送警报。
...
# 默认平台 (telegram 或 wechat)
Default_ExtNotify=wechat

# [新增] 通知过滤
# 定义哪些事件会触发消息发送。
# 可用选项: StateActivated (状态激活), StateCompleted (状态完成), Timeout (超时), StateInterrupted (状态中断), EntryDetected (检测到入口)
# 如果注释掉此行，将发送所有事件。
...
# 复杂链条：开始 -> (5s) -> 中间节点 -> (10s) -> 结束节点
Complex_Flow={StartNode, 5000, MidNode, 10000, EndNode, "复杂流程链"}
```
...
*   `--detailed-status`：打印关于活动状态、流转和计时器的详细信息。
